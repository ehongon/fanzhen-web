...
export default function PracticeTogetherPage() {
  ...
  useEffect(() => {
    ...
    const myTeams = allTeams.filter((t) => t.members.some((m) => m.userId === userId));
    setTeams(myTeams);
    if (myTeams.length > 0) {
    ...
    <div className="min-h-screen bg-ink">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: "即将开始", value: upcomingPractices.length, icon: Calendar, color: "text-blue-400" },
            { label: "进行中", value: ongoingPractices.length, icon: Clock, color: "text-gold" },
            { label: "已完成", value: completedPractices.length, icon: Zap, color: "text-emerald-400" },
            { label: "我参与的", value: myPracticeIds.size, icon: Users, color: "text-purple-400" },
          ].map((stat, index) => {
        ...
        {ongoingPractices.length === 0 && upcomingPractices.length === 0 && (
          ...
          >
            ...
            <h2 className="text-xl font-bold text-rice mb-2">暂无组队修炼</h2>
        ...
        {showCreateModal && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-lg mx-4 rounded-xl border border-gold/20 bg-ink p-6"
              onClick={(e) => e.stopPropagation()}
            >
              <h2 className="text-xl font-bold text-rice mb-4">发起组队修炼</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-rice/60 mb-2">选择道场</label>
                  <select
                    value={selectedTeamId}
                    onChange={(e) => setSelectedTeamId(e.target.value)}
                    className="w-full rounded-md border border-gold/20 bg-ink/50 text-rice px-3 py-2"
                  >
                    {teams.map((team) => (
                ...
                <div>
                  <label className="block text-sm text-rice/60 mb-2">功法名称</label>
                  <Input
                    value={gongfaName}
                ...
                <div>
                  <label className="block text-sm text-rice/60 mb-2">修炼时长（分钟）</label>
                ...
                <div>
                  <label className="block text-sm text-rice/60 mb-2">开始时间</label>
                  <Input
                    type="datetime-local"
                    value={scheduledTime}
                    onChange={(e) => setScheduledTime(e.target.value)}
                    className="bg-ink/50 border-gold/20 text-rice"
                  />
                </div>
