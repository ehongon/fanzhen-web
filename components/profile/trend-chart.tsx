...
interface TrendChartProps {
  type: "line" | "pie" | "bar" | "multiLine";
  title: string;
  data: LineChartPoint[] | PieChartSegment[] | LineChartPoint[][];
  className?: string;
  delay?: number;
}
...
// 多条折线图（用于身心状态趋势）
function MultiLineChart({ data, delay = 0 }: { data: LineChartPoint[][]; delay?: number }) {
  const colors = ["#c75b39", "#3b82f6", "#8b5cf6"];
  const labels = ["睡眠", "精力", "心情"];
  const allValues = data.flat();
  const maxValue = Math.max(...allValues.map((p) => p.value));
  const minValue = Math.min(...allValues.map((p) => p.value));
  const range = maxValue - minValue || 1;
  ...
  const getX = (index: number) =>
    padding.left + (index / (data[0].length - 1)) * chartWidth;
  const getY = (value: number) =>
    padding.top + chartHeight - ((value - minValue) / range) * chartHeight;
  ...
  return (
    ...
    >
      ...
      >
          ...
          return (
            <g key={lineIndex}>
              <motion.path
                ...
                initial={{ pathLength: 0 }}
                animate={{ pathLength: 1 }}
                transition={{ duration: 1.5, delay: delay + 0.2 + lineIndex * 0.2, ease: "easeInOut" }}
              />
              {lineData.map((p, i) => (
                <motion.circle
                  key={i}
                  ...
                  initial={{ opacity: 0, scale: 0 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ duration: 0.3, delay: delay + 0.5 + i * 0.03 }}
                />
              ))}
            </g>
          );
        ...
        {/* X轴标签 */}
        {data[0].map((p, i) => {
          if (i % Math.ceil(data[0].length / 6) !== 0 && i !== data[0].length - 1)
            return null;
          return (
            <text
              key={i}
  ...
  return (
    ...
    >
      <svg viewBox="0 0 200 200" className="w-40 h-40 sm:w-48 sm:h-48">
        ...
        <circle cx={centerX} cy={centerY} r="45" fill="hsl(var(--card))" />
        <text x={centerX} y={centerY - 5} textAnchor="middle" className="text-sm font-bold fill-foreground">
          总计
        </text>
        <text x={centerX} y={centerY + 15} textAnchor="middle" className="text-lg font-bold fill-primary">
          {total}h
        </text>
      </svg>
...
export default function TrendChart({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "bg-card rounded-xl border border-border/50 shadow-card p-4 sm:p-5",
        className
      )}
    >
      ...
      {type === "multiLine" && <MultiLineChart data={data as LineChartPoint[][]} delay={delay} />}
